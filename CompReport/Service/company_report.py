def get_formats(workbook):
    ...
    return {
        ...
        'Market_Cap': workbook.add_format({'size': 10, 'num_format': '#,##0.00', 'border': 1, 'bg_color': '#F3F7FF', 'align': 'center'}),
...
def company_report(df, latest_year):
    """生成公司财务报告"""
        ...
        for _, row in df.iterrows():
            ...
            for row_idx, (label, base_key) in enumerate(SI_items, start=1):
                ...
                if pd.isna(val):
                    ...
                    if type(val) == float:
                        worksheet.write(row_idx + 2, 2, round(val, 2), fmt['Market_Cap'])
                    else:
